...
% Last Modified by GUIDE v2.5 20-Mar-2013 07:41:19
...
set(handles.path,'String','H:\Project\SG_Project');
handles.new_test=0;
...
file_UART = fopen( strcat(folder_name,'\uart_tx\uart_tx_',num2str(handles.uart_txt_num),'.txt'),'w');
...
%payload = zeros(1,num_of_opcodes*3);
%payload_idx = 0;
...
%            payload(payload_idx + 1) = segment1;
%            payload(payload_idx + 2) = segment2;
%            payload(payload_idx + 3) = segment3;
%            payload_idx = payload_idx + 3;
...
% %UART from matlab to CP
% dataToSend=[sof     type    SG_reg_addr   length_msb      length_lsb       payload    crc     eof];
